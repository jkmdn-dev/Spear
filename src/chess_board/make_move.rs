use crate::{base_structures::{Move, Side}, CastleRight, ChessBoard, MoveFlag, Piece, Square};

impl ChessBoard {
    pub fn make_move(&mut self, mv: Move) {
        self.make_move_stm(mv, mv.get_from_square(), mv.get_to_square())
    }

    #[inline]
    fn make_move_stm(&mut self, mv: Move, from_square: Square, to_square: Square) {
        if self.side_to_move() == Side::WHITE {
            self.make_move_move_flag::<true, false>(mv, from_square, to_square)
        } else {
            self.make_move_move_flag::<false, true>(mv, from_square, to_square)
        }
    }

    #[inline]
    fn make_move_move_flag<const STM_WHITE: bool, const NSTM_WHITE: bool>(&mut self, mv: Move, from_square: Square, to_square: Square) {
        match mv.get_flag() {
    ...
    #[inline]
    fn make_move_moved_piece<const STM_WHITE: bool, const NSTM_WHITE: bool, const MOVE_FLAG: u16>(&mut self, mv: Move, from_square: Square, to_square: Square) {
        let moved_piece = self.get_piece_on_square(from_square);
        match moved_piece {
            Piece::PAWN => self.make_move_captrued_piece::<STM_WHITE, NSTM_WHITE, MOVE_FLAG, { PAWN }>(mv, from_square, to_square),
            Piece::KNIGHT => self.make_move_captrued_piece::<STM_WHITE, NSTM_WHITE, MOVE_FLAG, { KNIGHT }>(mv, from_square, to_square),
            Piece::BISHOP => self.make_move_captrued_piece::<STM_WHITE, NSTM_WHITE, MOVE_FLAG, { BISHOP }>(mv, from_square, to_square),
            Piece::ROOK => self.make_move_captrued_piece::<STM_WHITE, NSTM_WHITE, MOVE_FLAG, { ROOK }>(mv, from_square, to_square),
            Piece::QUEEN => self.make_move_captrued_piece::<STM_WHITE, NSTM_WHITE, MOVE_FLAG, { QUEEN }>(mv, from_square, to_square),
            Piece::KING => self.make_move_captrued_piece::<STM_WHITE, NSTM_WHITE, MOVE_FLAG, { KING }>(mv, from_square, to_square),
            Piece::NONE => self.make_move_captrued_piece::<STM_WHITE, NSTM_WHITE, MOVE_FLAG, { NONE }>(mv, from_square, to_square),
            _ => unreachable!()
        }
    }

    #[inline]
    fn make_move_captrued_piece<const STM_WHITE: bool, const NSTM_WHITE: bool, const MOVE_FLAG: u16, const MOVED_PIECE: u8>(&mut self, mv: Move, from_square: Square, to_square: Square) {
        if MOVE_FLAG & MoveFlag::CAPTURE == 0 {
            self.make_move_internal::<STM_WHITE, NSTM_WHITE, MOVE_FLAG, MOVED_PIECE, { NONE }>(mv, from_square, to_square);
            return;
        } 

        let captured_piece = self.get_piece_on_square(to_square);
        match captured_piece {
            Piece::PAWN => self.make_move_internal::<STM_WHITE, NSTM_WHITE, MOVE_FLAG, MOVED_PIECE, { PAWN }>(mv, from_square, to_square),
            Piece::KNIGHT => self.make_move_internal::<STM_WHITE, NSTM_WHITE, MOVE_FLAG, MOVED_PIECE, { KNIGHT }>(mv, from_square, to_square),
    ...
    fn make_move_internal<const STM_WHITE: bool, const NSTM_WHITE: bool, const MOVE_FLAG: u16, const MOVED_PIECE: u8, const CAPTURED_PIECE: u8>(&mut self, mv: Move, from_square: Square, to_square: Square) {
