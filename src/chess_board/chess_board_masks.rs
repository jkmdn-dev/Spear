use crate::{attacks::{Attacks, Rays}, base_structures::Side, Bitboard, ChessBoard, Piece};
...
impl ChessBoard {
    ...
    pub fn generate_checkers_mask(&mut self) {
        self.masks.checkers = self.all_attackers_to_square(self.get_occupancy(), self.get_king_square(self.side_to_move()), self.side_to_move().flipped());
    }
    ...
    pub fn generate_ortographic_pins_mask(&self) -> Bitboard {
        ...
        let relevant_pieces =
        self.get_piece_mask_for_side(Piece::ROOK, attacker_color) | self.get_piece_mask_for_side(Piece::QUEEN, attacker_color);
        let potential_pinners =
            Attacks::get_rook_attacks_for_square(king_square, self.get_occupancy_for_side(attacker_color)) & relevant_pieces;
    ...
    pub fn generate_diagonal_pins_mask(&self) -> Bitboard {
        ...
        let king_square = self.get_king_square(self.side_to_move());
        let relevant_pieces =
        self.get_piece_mask_for_side(Piece::BISHOP, attacker_color) | self.get_piece_mask_for_side(Piece::QUEEN, attacker_color);
        let potential_pinners =
            Attacks::get_bishop_attacks_for_square(king_square, self.get_occupancy_for_side(attacker_color)) & relevant_pieces;
        let mut result = Bitboard::EMPTY;
        potential_pinners.map(|potential_pinner| {
            let ray = Rays::get_ray(king_square, potential_pinner);
            if (ray & self.get_occupancy_for_side(self.side_to_move())).only_one_bit() {
    ...
    pub fn generate_attack_map(&self, attacker_side: Side) -> Bitboard {
        ...
        (attacker_pieces & self.get_piece_mask(Piece::KING)).map(|king_square| {
            threats |= Attacks::get_king_attacks_for_square(king_square)
        });
    
        (attacker_pieces & self.get_piece_mask(Piece::KNIGHT)).map(|knight_square| {
